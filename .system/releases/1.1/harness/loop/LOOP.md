...
The closed loop, wired and running as **candidates** in `apps/_drafts/`. Every input
...
node harness/loop/reconciler/index.mjs            # raises any limbo signal
node governance/checks/no-ghost-agent.mjs        # proves no fabricated targets
```
