...
import {
  openSync, cpSync, existsSync, lstatSync, mkdirSync, mkdtempSync, readFileSync, readlinkSync,
...
const CLOSURE = [
  ...
  '.system/bin',          // this publisher + the boot launcher (census row .system/bin/os-publish.mjs)
  '.claude',              // the HOST WIRING (settings.json hooks/statusLine + commands) — the boot
                          // contract is part of the release; the live copy stays at the repo root
                          // because Claude Code only loads it there (settings.local.json excluded)
]
...
function assembleSnapshot(version, { from = REPO } = {}) {
  ...
  const filter = (src) => {
    ...
    if (segs.at(-1) === 'settings.local.json') return false // machine-local, never sealed
    return true
  }
  ...
  // EXPLICIT INCLUSION LIST: every file sealed into this release, relative to
  // the snapshot root — the definitive answer to "which workbench files are
  // part of the booted OS". Written both into SNAPSHOT.json and as FILES.txt.
  const files = []
  const walk = (dir, base) => {
...
function main() {
  ...
  // publish — serialized by an O_EXCL lock: two concurrent publishes must not
  // race staging/rename/repoint (X3). The loser refuses loudly; a dead holder's
  // lock (stale mtime) is broken.
  ...
  try {
    ...
    let stale = false
    try { stale = (Date.now() - lstatSync(lockPath).mtimeMs) > 120000 } catch { stale = true }
    if (!stale) die('another publish holds the lock (.system/releases/.publish.lock) — publishes are serialized; retry when it finishes')
  ...
  // Refuse EARLY if the pointer is a real directory: repoint's rename cannot
  // replace a directory with a symlink (EISDIR) and would otherwise fail AFTER
  // the destructive reseal steps.
  {
    const dst = join(RELEASES, channel)
    if (!isSymlink(dst) && existsSync(dst) && lstatSync(dst).isDirectory()) {
      die(`pointer '${channel}' is a real DIRECTORY (copied-in deployment) — move it aside before publishing; repointing over a directory is not atomic`)
    }
  }
  let version
  ...
  // Reseal without a destruction window: rename the OLD tree aside (atomic),
  // land the new one (atomic), and only then discard the old. A crash between
  // the two renames is repaired by renaming the .old dir back — `current`
  // never dangles for O(tree-deletion) (peer-review P2 finding, reproduced).
  const oldAside = `${dest}.old-${process.pid}`
  if (existsSync(dest)) renameSync(dest, oldAside)
  renameSync(staging, dest)
  // RE-verify at the FINAL location before repointing: rename-invariance is
  // false (proven live — a tracked snapshot path changes what the in-image
  // doctor's git-baselined checks see vs an untracked .staging dir). What gets
  // verified must be exactly what gets booted.
  const finalProblems = verifySnapshot(dest)
  if (finalProblems.length) {
    rmSync(dest, { recursive: true, force: true })
    if (existsSync(oldAside)) renameSync(oldAside, dest) // restore — the channel pointer stays valid
    die(`post-rename verification failed at the FINAL location — old snapshot restored, '${channel}' NOT repointed:\n  • ${finalProblems.join('\n  • ')}`)
  }
