...
function main() {
  ...
  try {
    ...
    process.stdin.on('end', () => {
      ...
      // LOCKDOWN (os-reshape P4, approved in the execution plan): BOTH tiers
      // enforce — union of blocks. decide() keeps the sibling-project
      // blocklist (reads included); decideStrict() is the fail-CLOSED write
      // allowlist — a WRITE outside this repo is refused even when the guard
      // cannot fully resolve it.
      const r = decide(payload)
      if (r.block) { process.stderr.write('⛔ ' + r.reason + '\n'); return done(2) }
      const rs = decideStrict(payload)
      if (rs.block) { process.stderr.write('⛔ ' + rs.reason + '\n'); return done(2) }
      done(0)
    })
