...
| harness-doctor | 0.3.3 | staging | governance | `governance/checks/doctor.mjs` | Drift-check: validates this manifest against disk, wiring, environment, and the production-never-depends-on-sandbox rule |
...
| loop-store | 0.1.2 | candidate | store | `harness/loop/loop-store/` | The data layer: append-only JSONL truth per stream → readable state/os.db; sole-writer per stream |
...
| confinement | 0.3.2 | candidate | gate | `harness/guard/confinement/` | PreToolUse fence: blocks tool calls targeting sibling projects (preventive tier; hardening tracked in the confinement-sandbox spec) |
...
| handoff | 0.1.2 | candidate | governance | `apps/_drafts/handoff/` | Deterministic handoff spine: guarantees session handoffs are saved to record/handoffs/ (+ renders docs/RESUME-HERE.md); new/check/list. |
| incident | 0.1.2 | candidate | governance | `apps/_drafts/incident/` | Deterministic spine of the incident log: STEPS + missingSteps/isComplete + new/check/list |
| os-publish | 0.1.0 | candidate | gate | `.system/bin/os-publish.mjs` | Sealed-runtime publisher: assembles the release snapshot (.system/releases/<v>/), CUT-THEN-VERIFY (doctor + no-ghost + hostile boot smoke INSIDE the snapshot), atomically repoints current|next; --verify + --boot-check (the boot banner). Fail-closed control. |
...
| reshape-rig | 0.1.3 | candidate | gate | `apps/_drafts/reshape-rig/` | Migration test rig for the os-reshape plan: golden-master capture/compare (F5), the §D2 eval battery, fault-injection rigs, the sealed-boot S-matrix (RED until P2), concurrency rigs (RED until P3), scoped grep-gate |
