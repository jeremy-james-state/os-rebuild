#!/usr/bin/env node
/**
 * reshape-rig/rig.mjs — CANDIDATE (apps/_drafts/), not admitted.
 *
...
export const PATHS = {
  sessionFeedback: 'harness/loop/session-feedback/index.mjs',
  statusline: 'harness/loop/session-feedback/statusline.mjs',
  orchestrator: 'harness/loop/orchestrator/index.mjs',
  confinement: 'harness/guard/confinement/index.mjs',
  classifierDir: 'harness/loop/classifier',
  orchestratorDir: 'harness/loop/orchestrator',
  loopStore: 'harness/loop/loop-store/index.mjs',
  doctor: 'governance/checks/doctor.mjs',
  governanceCheck: 'governance/checks/governance-check.mjs',
  structureCheck: 'governance/checks/structure-check.mjs',
  noGhost: 'governance/checks/no-ghost-agent.mjs',
...
export function repoCopy({ withGit = true, from = REPO } = {}) {
  ...
  const filter = (src) => {
    ...
    // Copies start UNSEALED: skip snapshot dirs + channel pointers (keep the
    // *.json pin files) so S-matrix tests exercise a fresh publish, not the
    // workbench's sealed release (releases are immutable — a copy re-publish
    // of an existing version would be refused).
    if (segs[0] === '.system' && segs[1] === 'releases' && segs.length > 2 && !segs[2].endsWith('.json')) return false
    return true
  }
