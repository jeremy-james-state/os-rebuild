...
| os-publish | 0.1.1 | candidate | gate | `.system/bin/os-publish.mjs` | Sealed-runtime publisher: assembles the release snapshot (.system/releases/<v>/), CUT-THEN-VERIFY (doctor + no-ghost + hostile boot smoke INSIDE the snapshot), atomically repoints current|next; --verify + --boot-check (the boot banner). Fail-closed control. |
...
| reshape-rig | 0.1.4 | candidate | gate | `apps/_drafts/reshape-rig/` | Migration test rig for the os-reshape plan: golden-master capture/compare (F5), the §D2 eval battery, fault-injection rigs, the sealed-boot S-matrix (RED until P2), concurrency rigs (RED until P3), scoped grep-gate |
