#!/usr/bin/env node
// governance/checks/doctor.mjs — the harness drift-check (self-policing).
//
// Compares the declared harness (harness/manifest.json — rails + the full
// component census, one spine file) against reality:
...
//
// Zero-dependency. Run: node governance/checks/doctor.mjs [--inventory] [--json]

import { realpathSync, readFileSync, existsSync, readdirSync, statSync } from 'node:fs'
...
const KNOWN_NON_HARNESS = new Set([
  '.system', 'skills', 'web', 'handoffs', 'foundation', 'templates', 'requirements',
...
// Self-admission guard, post-reshape geometry (lifecycle state DECOUPLED from location):
// apps/_drafts/ is the pre-admission zone for apps — a row whose path lives there must be
// state=candidate or state=sandbox; an admitted state (production/staging) under _drafts/ is
// self-admission drift. Kernel parts (harness/loop|guard/) MAY be state=candidate in place —
// their admission is the human manifest state flip (CODEOWNERS-gated), not a file move
...
export function checkSandboxContainment(manifest) {
  ...
  for (const c of manifest.components || []) {
    ...
    if (p.startsWith('apps/_drafts/') && !PRE_ADMISSION_STATES.has(c.state)) {
      out.push(finding('ERROR', 'drafts-path-non-candidate-state',
        `Component '${c.id}' lives under apps/_drafts/ but declares state='${c.state}'.`,
        `A _drafts component must be state=candidate or state=sandbox (both pre-admission). Admission moves an app out of apps/_drafts/ AND flips its manifest state under review (governance/rules/harness-admission.md) — never self-admit by flipping state in place.`))
    }
  }
  return out
}
...
// the definition). manifest.json validates against its schema; every
...
// harness/index.md is GENERATED from the manifest components — it must byte-match renderIndex(). Fail-closed,
...
export function checkIndexSync(manifest, root) {
  ...
  if (readFileSync(p, 'utf8') !== expected) {
    out.push(finding('ERROR', 'index-stale', `harness/index.md is out of sync with manifest.json.`, `Run: node harness/render.mjs --index (and commit).`))
  }
  return out
}
  ...
  for (const c of manifest.components || []) {
    ...
    if (readFileSync(p, 'utf8') !== renderChangelog(c)) {
      out.push(finding('ERROR', 'changelog-stale',
        `${c.path}CHANGELOG.md is out of sync with its manifest versions[].`,
...
export function checkReleaseConsistency(manifest, root) {
  ...
  if (!hv) return out
  const p = join(root, '.system', 'releases', `${hv}.json`)
  if (!existsSync(p)) {
    out.push(finding('ERROR', 'release-missing',
      `No release record .system/releases/${hv}.json for harnessVersion '${hv}'.`,
      `Cut the release (pin the component set), or fix harnessVersion.`))
    return out
  }
  let rel
  try { rel = JSON.parse(readFileSync(p, 'utf8')) } catch (e) {
    out.push(finding('ERROR', 'release-unreadable', `.system/releases/${hv}.json: ${e.message}`, 'Fix the release JSON.'))
    return out
  }
  const pins = rel.pins || {}
  for (const c of manifest.components || []) {
    ...
    if (pins[c.id] !== c.version) {
      out.push(finding('ERROR', 'release-pin-drift',
        `Release ${hv} pins '${c.id}'=${pins[c.id] ?? '(absent)'} but the manifest has ${c.version}.`,
...
//
// Baseline = the highest 'harness-vX.Y' tag; its pins live in .system/releases/<X.Y>.json. For each
...
export function checkVersionBumpOnChange(manifest, root) {
  const out = []
  // Baseline-integrity guard: this check is only meaningful when `root` IS the
  // git toplevel. A sealed snapshot nested inside the workbench repo would
  // otherwise inherit the parent's tags/index and read its own (post-tag-
  // committed) files as drift — flipping the verification verdict on a
  // byte-identical image depending on commit/tag ordering (peer-review P2,
  // reproduced). Not-toplevel or no-git → fail open, same as a no-git deploy.
  try {
    const top = execFileSync('git', ['-C', root, 'rev-parse', '--show-toplevel'], { encoding: 'utf8' }).trim()
    if (realpathSync(top) !== realpathSync(root)) return []
  } catch { return [] }
  ...
  let pins
  try {
    const rel = JSON.parse(readFileSync(join(root, '.system', 'releases', `${version}.json`), 'utf8'))
    pins = rel.pins || {}
  } catch { return [] }
  ...
  for (const c of manifest.components || []) {
    ...
    if (substantive.length && c.version === pins[c.id]) {
      out.push(finding('ERROR', 'version-bump-required',
        ...
        `Bump ${c.id}'s version in manifest.json, append a versions[] entry, and run node harness/render.mjs --changelogs.`))
    }
  }
  return out
}
...
export function runDoctor({ root = DEFAULT_ROOT, manifestPath = DEFAULT_MANIFEST } = {}) {
  let manifest
  try {
  ...
  // One spine file: rails + the full component census live in manifest.json
  // (the manifest census was merged in at the os-reshape P1 merge). An absent/empty
  // components array is NOT fail-open — checks treat it as a real census.
  const merged = { ...manifest, components: manifest.components || [] }
  const findings = [
    ...checkSchemas(root),
