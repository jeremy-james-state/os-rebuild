...
- **Reads:** Supabase `public.osr_events` (a hosted projection) with the **public anon key**
  under RLS (select-only). Truth lives in `record/*.jsonl` in git; `osr_events` is rebuildable.
...
state/os.db (readable SQLite)
      ...
      ▼
Supabase public.osr_events  (RLS: public read)
      │ anon key (public)
      ▼
this dashboard
```
