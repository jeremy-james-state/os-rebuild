 ...
 *   node scripts/data-lock.mjs verify   # re-checksum vs the baseline → identical or DRIFT
 *   node scripts/data-lock.mjs unlock   # chflags nouchg + restore write bits (data stays appendable)
 *
...
function main() {
  ...
  if (cmd === 'lock') {
    ...
    console.log('usage: node scripts/data-lock.mjs <lock|verify|unlock> [root]')
    process.exitCode = 2
  }
}
