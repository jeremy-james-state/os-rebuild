#!/usr/bin/env node
/**
 * harness-lock — CANDIDATE (harness/sandbox/), not admitted. See
...
export function componentOf(p, { cwd = REPO, root = REPO } = {}) {
  ...
  if (!under(abs, join(root, 'harness'))) return null
  const rest = abs.slice(join(root, 'harness').length).split(sep).filter(Boolean)
  // rest = [<type-or-sandbox>, <component>, ...] — sandbox adds one level of nesting.
  if (rest[0] === 'sandbox') return rest[1] || null
  return rest[1] || null // e.g. runners/<component>, lib/<component>, orchestrators/<component>
}
...
export function writeTargets(tool, input = {}) {
  ...
  if (typeof tool === 'string' && tool.startsWith('mcp__')) {
    // an mcp tool with any string arg that looks like a harness/ path
    const vals = Object.values(input || {}).filter((v) => typeof v === 'string')
    return vals.filter((v) => v.includes('harness/') || v.includes('harness' + sep))
  }
  if (tool === 'Bash') {
    const cmd = String(input.command || '')
    // Only flag commands that plausibly WRITE under harness/: a redirection, or a known mutating
    // command with a harness/ argument. Reads (cat/grep/node --test) are NOT writes → [].
    const out = []
    // `> harness/...` or `>> harness/...`
    for (const m of cmd.matchAll(/>>?\s*("?)((?:\.\/)?harness\/[^\s'"|;&)]+)\1/g)) out.push(m[2])
    // mutating verbs with a harness/ path arg (tee/cp/mv/sed -i/touch/rm/mkdir)
    if (/\b(tee|cp|mv|touch|rm|mkdir|sed\s+-i)\b/.test(cmd)) {
      for (const m of cmd.matchAll(/(?:\s|^)((?:\.\/)?harness\/[^\s'"|;&)]+)/g)) out.push(m[1])
    }
    return out
  }
  return []
