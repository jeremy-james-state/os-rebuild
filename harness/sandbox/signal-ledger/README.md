...
> Lives in `harness/sandbox/`. It **is not the harness** — it is a candidate at step
...
```bash
node harness/sandbox/signal-ledger/ledger.mjs            # status + gaps
node harness/sandbox/signal-ledger/ledger.mjs rebuild    # rebuild state/os.db
node harness/sandbox/signal-ledger/view.mjs --out /tmp/flow.html
node --test harness/sandbox/signal-ledger/*.test.mjs
```
