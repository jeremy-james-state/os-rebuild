...
/** The release version tokens present as pin files in harness/releases/ (e.g. ['0.7','0.8']). Fail-open → []. */
function releaseVersions(root) {
  try {
    return readdirSync(join(root, 'harness/releases'))
...
export function gitDrift({ root = REPO_ROOT, git = realGit(root) } = {}) {
  ...
  try {
    ...
    try {
      ...
      const pinExists = hv && existsSync(join(root, `harness/releases/${hv}.json`))
      if (pinExists) {
