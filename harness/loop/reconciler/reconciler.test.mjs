...
// Build a tmp root with a repo layout: a manifest at `hv` and .system/releases pin files for `releases`.
function tmpRoot({ hv = null, releases = [] } = {}) {
  ...
  for (const v of releases) writeFileSync(join(root, `.system/releases/${v}.json`), JSON.stringify({ harnessVersion: v }))
  return root
}
