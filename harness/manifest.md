...
**Harness version:** 0.8.0 · **Manifest format:** 2.0 · **Updated:** 2026-06-30

## Boundary

- **Model:** directory+manifest, one branch
- **Production:** harness/ (folders are TYPES: orchestrators/ runners/ services/ hooks/ lib/)
- **Sandbox:** none — promotion state (production|staging|sandbox|planned|quarantined|retired) is a manifest field, not a folder
...
| Component | Type | Kind | Path | Role |
| --- | --- | --- | --- | --- |
| approval-runner | runner | governance | `harness/runners/approval-runner/` | Turns template/git/host-capability signals into durable approval records (SQLite + JSONL); exact-file commits on session branches only  ⚠ no read API / unified violation dashboard yet (Law 11 partial) |
| approve-hook | hook | hook | `harness/hooks/approve-hook/` | UserPromptSubmit hook intercepting 'approve <item> <stage>' prompts  ⚠ built but never wired; untested |
| auto-mode | orchestrator | runtime | `harness/orchestrators/auto-mode/` | Away-work runner: select reversible/valuable units -> playback -> runtime select -> fan out thin workers under a concurrency cap  ⚠ approve-mode trusted; tool-use execution trust still pending (Gate B) |
| away-gate | hook | hook | `harness/hooks/away-gate/` | PreToolUse safety gate for Auto Mode: allow read/test/git-safe/draft, deny deploy/external-send/money/destructive  ⚠ snippet exists but not merged into an active settings.json -> not wired by default |
| build-templates | library | tool | `harness/lib/build-templates/` | Bounded catalog of deterministic codegen templates |
| builder | orchestrator | engine-agent | `harness/orchestrators/builder/` | Coordinates plan execution: build-gate -> provision -> topo-sort -> per-step execute/verify/retry/reframe/escalate -> acceptance |
| capture-hook | hook | hook | `harness/hooks/capture-hook/` | UserPromptSubmit hook for +task / +list instant capture (no model turn)  ⚠ untested |
| chain-state-attest | runner | governance | `harness/runners/chain-state-attest/` | Deterministic digest over STATUS chain/pre-frame/queue; blocks chain edits unless attestation matches HEAD |
| clarifier | runner | engine-agent | `harness/runners/clarifier/` | Transcript/request -> grounded End State (goal, conditions, assumptions, definition of done)  ⚠ two meanings stores (clarifier.db vs confirmed-meanings.db) pending unification |
...
| context-window-monitor | service | observability | `harness/services/context-window-monitor/` | Polls Claude/Codex transcript telemetry; adaptive rotation thresholds; writes trigger events; can auto-prepare handoff |
| decision-enforcer | runner | governance | `harness/runners/decision-enforcer/` | Pre-push gate: status truth, session registry, live-path, branch ownership, post-handoff, write-branch, template conformance, chain-state attestation  ⚠ only fires if core.hooksPath is configured; not configured in fresh clones / cloud |
| decomposer | runner | engine-agent | `harness/runners/decomposer/` | Top-down task-tree builder: confidence gate -> cut -> atomicity -> depth budget -> store -> dependencies |
| deployer | runner | engine-agent | `harness/runners/deployer/` | Deploy gate + sign-off + verify + auto-rollback; auto only for preview/staging/non-client/low-impact  ⚠ built but not wired into a live flow; no formal Deploy stage contract yet |
| executor | runner | engine-agent | `harness/runners/executor/` | Executes one atomic step: mechanical -> template codegen, novel -> model; confines writes to worktree |
...
| reducer | runner | store | `harness/runners/reducer/` | Stateless read model: derives current unit state and parent/child tree from the event log |
...
| repo-hygiene | runner | governance | `harness/runners/repo-hygiene/` | Audits and prunes merged session branches/worktrees |
| researcher | runner | experiment | `harness/runners/researcher/` | Discovery/research lane: grounded knowledge with cited sources  ⚠ schema only; no agent entry point; unwired |
...
| scoper | runner | engine-agent | `harness/runners/scoper/` | End State -> bounded, viable Scope (in/out items with traceability) |
| session-governor | runner | governance | `harness/runners/session-governor/` | Active-session registry, duplicate-topic collision guard, push visibility, post-handoff write caution |
| session-start-hook | hook | hook | `harness/hooks/session-start-hook/` | SessionStart: validate workspace binding, fail-closed on wrong checkout, inject continuity packet  ⚠ hardcodes local path -> fails closed in cloud context; must move to repo-identity (both-first-class decision) |
| spike | runner | experiment | `harness/runners/spike/` | Ad-hoc worker stub |
| store | service | store | `harness/services/store/` | Append-only event store (work.db) + observation store (obs.db: flags, agent_runs, chain_runs) |
...
| template-conformance | runner | governance | `harness/runners/template-conformance/` | Frame/Scope/Plan must conform to templates; 4 visibility states (hidden-structural, hidden-needs-agent-review, ready-for-human-review, approved); spillover routing |
| tester | runner | engine-agent | `harness/runners/tester/` | Whole-build acceptance: rules + deterministic check-generation + adversarial subagent review; never model-verifies |
...
| write-branch-audit | runner | governance | `harness/runners/write-branch-audit/` | Asserts the active session branch matches the git write branch |
