...
function tmpRoot() {
  ...
  mkdirSync(join(root, 'apps', '_drafts', 'target'), { recursive: true })
  return root
}
...
const writeToTarget = (sid) => ({
  tool_name: 'Write',
  tool_input: { file_path: 'apps/_drafts/target/index.mjs' },
  session_id: sid,
})
...
test('writeTargets: writes counted, reads/other ignored', () => {
  assert.deepEqual(writeTargets('Write', { file_path: 'apps/_drafts/x/a.mjs' }), ['apps/_drafts/x/a.mjs'])
  assert.deepEqual(writeTargets('Read', { file_path: 'apps/_drafts/x/a.mjs' }), [])
  assert.deepEqual(writeTargets('Bash', { command: 'node --test apps/_drafts/x/x.test.mjs' }), [])
  assert.ok(writeTargets('Bash', { command: 'echo hi > apps/_drafts/x/a.txt' }).length === 1)
})
...
test('read of a locked component → allow (reads never block)', () => {
  ...
  try {
    ...
    const r = decide({ tool_name: 'Read', tool_input: { file_path: 'apps/_drafts/target/index.mjs' }, session_id: 'B' }, { root, now: NOW, pidAlive: alive })
    assert.equal(r.block, false)
  } finally { rmSync(root, { recursive: true, force: true }) }
})
...
test('decideBranchDiscipline: on main, write to harness/ → BLOCK (fail-closed)', () => {
  const r = decideBranchDiscipline(archWrite('apps/_drafts/target/index.mjs'), { root: '/r', branch: 'main' })
  assert.equal(r.block, true)
...
test('decideBranchDiscipline: on a work/foo branch → all allow', () => {
  assert.equal(decideBranchDiscipline(archWrite('apps/_drafts/target/index.mjs'), { root: '/r', branch: 'work/foo' }).block, false)
